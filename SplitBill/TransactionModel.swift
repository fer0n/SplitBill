...
struct Transaction: Identifiable, Equatable, Codable {
    ...
    var hasOnlyOneNotManuallyAdjustedShare: Bool {
        let manuallyAdjustedShares = shares.filter { !$0.value.manuallyAdjusted }
        return manuallyAdjustedShares.count == 1
    }
    ...
    mutating func refreshShares() throws {
        ...
        for (id, share) in shares where !share.manuallyAdjusted {
            count -= 1
    ...
    func splitAmount(amount: Double, numParts: Int) throws -> [Double] {
        ...
        let roundedAmount = round(amount * 100)
        if roundedAmount > Double(Int.max) {
            throw EditShareError.numberTooLarge
        }
        var result: [Double] = []
        var remainder = Int(roundedAmount) // throw away more than two decimals
        for index in stride(from: numParts, to: 0, by: -1) {
            let res = remainder / index
            remainder -= res
